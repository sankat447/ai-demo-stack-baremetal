...
export async function handleDcTrackPortsTool(toolName, args) {
    ...
    switch (toolName) {
        ...
        case 'dctrack_create_data_port': {
            ...
            if (p.connector)
                portData.connector = p.connector;
            if (p.mediaType) {
                portData.media = p.mediaType;
                portData.mediaType = p.mediaType;
            }
            if (p.protocol)
                portData.protocol = p.protocol;
            if (p.dataRate)
                portData.dataRate = p.dataRate;
...
//# sourceMappingURL=ports-handler.js.map