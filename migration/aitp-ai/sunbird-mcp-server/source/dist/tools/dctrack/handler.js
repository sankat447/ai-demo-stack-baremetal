 ...
 */
function wrapWithPagination(data) {
...
export async function handleDcTrackReadTool(toolName, args) {
    ...
    switch (toolName) {
        ...
        case 'dctrack_get_cabinet_items': {
            ...
            if (!cabItemsId && cabArgs.cabinetName) {
                ...
                if (found?.id)
                    cabItemsId = Number(found.id);
                else
                    throw new Error(`Cabinet "${cabArgs.cabinetName}" not found`);
            }
            if (!cabItemsId)
                throw new Error('Either cabinetId or cabinetName is required');
        ...
        case 'dctrack_get_cabinet_u_map': {
            ...
            if (!uMapCabId && uMapArgs.cabinetName) {
                ...
                const found = results[0];
                if (found?.id)
                    uMapCabId = Number(found.id);
                else
                    throw new Error(`Cabinet "${uMapArgs.cabinetName}" not found`);
            }
            if (!uMapCabId)
                throw new Error('Either cabinetId or cabinetName is required');
        ...
        case 'dctrack_get_cabinet_capacity': {
            ...
            if (!cabId && cap.cabinetName) {
                ...
                const found = results[0];
                if (found?.id)
                    cabId = Number(found.id);
            }
            if (!cabId)
                throw new Error('Provide either cabinetId or a valid cabinetName');
        ...
        case 'dctrack_list_cabinets_with_capacity': {
            ...
            const results = await Promise.all(cabinets.map(async (cab) => {
                ...
                return {
                    ...
                    installedItems: capacity?.installedItems ?? 0,
                };
            }));
