...
export async function handleDcTrackTicketsTool(toolName, args) {
    ...
    switch (toolName) {
        ...
        case 'dctrack_create_ticket': {
            ...
            delete clean.cmbPriority;
            delete clean.severity;
            delete clean.cmbSeverity;
            if (parsed.priority) {
                clean.description = (clean.description || clean.summary || '') + ' [Priority: ' + parsed.priority + ']';
            }
            if (parsed.ticketPurpose) {
                clean.cmbTicketPurpose = parsed.ticketPurpose;
                clean.ticketPurpose = parsed.ticketPurpose;
            }
            if (parsed.ticketType) {
                clean.cmbTicketType = parsed.ticketType;
                clean.ticketType = parsed.ticketType;
            }
            if (parsed.location) {
                clean.cmbLocation = parsed.location;
                clean.location = parsed.location;
            }
...
//# sourceMappingURL=tickets-handler.js.map