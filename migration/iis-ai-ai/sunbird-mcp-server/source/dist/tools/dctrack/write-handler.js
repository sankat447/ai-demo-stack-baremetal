 ...
 */
async function resolveItemId(itemId, itemName) {
    if (itemId)
        return itemId;
    if (!itemName)
        throw new Error('Either itemId or itemName is required');
    const results = await dctrackClient.searchItems({ name: itemName, pageSize: 1 });
    const found = results[0];
    if (!found?.id)
        throw new Error(`Item "${itemName}" not found in dcTrack`);
    return Number(found.id);
}
export async function handleDcTrackWriteTool(toolName, args) {
    ...
    switch (toolName) {
        ...
        case 'dctrack_move_item': {
            ...
            const targetCab = p.targetCabinetName || p.targetCabinetId || (() => { throw new Error('Either targetCabinetId or targetCabinetName is required'); })();
        ...
        case 'dctrack_create_connection': {
            ...
            if (!conn.sourceItemId)
                throw new Error('Either sourceItemId or sourceItemName is required');
            if (!conn.destItemId)
                throw new Error('Either destItemId or destItemName is required');
            return dctrackClient.createConnection(conn);
        }
