...
// Rack Summary
// ---------------------------------------------------------------------------
export async function getRackSummary(args) {
    let cabinet;
    let cabinetId;
    if (args.cabinetId) {
        ...
        cabinet = cabinets.find((c) => (c.tiName ?? c.name ?? '').toLowerCase() === args.cabinetName.toLowerCase());
        if (cabinet)
            cabinetId = Number(cabinet.id);
    }
    ...
    const pduReadings = await Promise.all(pdus.map(async (pdu) => ({
        pdu,
    ...
    const totalPowerKw = pduReadings.reduce((sum, pr) => sum + ((pr.readings?.activePower ?? 0) / 1000), 0);
    const avgTemp = sensorReadings.temperature.length > 0
...
export async function findCapacity(params) {
    ...
    for (const cab of cabinets) {
        const cabId = Number(cab.id);
        const cap = await dctrackClient.getCabinetCapacity(cabId);
        if (!cap)
            continue;
        const ruHeight = Number(cab.tiRUs ?? cab.ruHeight ?? 42);
        ...
        if (availU >= params.requiredU && availPwr >= params.requiredPowerKw) {
            ...
            available.push({
                cabinetId: cabId, cabinetName: cab.tiName ?? cab.name, location: cab.cmbLocation ?? cab.locationName,
...
// Health Status
// ---------------------------------------------------------------------------
export async function getHealthStatus(params) {
...
// ---------------------------------------------------------------------------
export async function identifyGhostServers(params) {
    ...
    for (const item of items) {
        const pd = itDevices.find((d) => d.name.toLowerCase() === item.tiName.toLowerCase() || d.cabinetName === item.cmbCabinet);
...
// Power Chain
// ---------------------------------------------------------------------------
export async function getPowerChain(params) {
...
export async function thermalAnalysis(params) {
    const sensors = await poweriqClient.listSensors({ sensorType: 'temperature', cabinetId: params.cabinetId });
    const sensorReadings = await Promise.all(sensors.map(async (s) => ({ sensor: s, reading: await poweriqClient.getSensorReadings(s.id) })));
    ...
    if (params.includeRecommendations && hotspots.length > 0) {
        ...
        if (hotspots.length > 3)
            recs.push('Multiple hotspots detected - review CRAC/CRAH capacity');
        if (avg && avg > 24)
            recs.push('Average temperature elevated - consider lowering supply air temperature');
