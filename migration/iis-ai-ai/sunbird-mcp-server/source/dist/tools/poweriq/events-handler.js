...
export async function handlePowerIQEventsTool(toolName, args) {
    ...
    switch (toolName) {
        case 'poweriq_list_events': {
            ...
            const params = {};
            if (p.severity)
                params['severity_eq'] = p.severity;
            if (p.eventable_type)
                params['eventable_type_eq'] = p.eventable_type;
            if (p.eventable_id)
                params['eventable_id_eq'] = p.eventable_id;
            if (p.limit)
