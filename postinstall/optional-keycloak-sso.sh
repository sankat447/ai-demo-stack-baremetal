#!/usr/bin/env bash
# OPTIONAL — Keycloak OIDC SSO for the OCP console (+ htpasswd break-glass).
#
# NOT the chosen path: the plan is keep kubeadmin now and wire Active Directory
# (LDAP) later — see postinstall/04-identity.sh. Retained as a ready alternative
# in case AD integration slips and you'd rather do SSO via the in-stack Keycloak.
# It does NOT delete kubeadmin.
#
...
NOTE: kubeadmin is intentionally KEPT (per the current plan). Do not delete it.
